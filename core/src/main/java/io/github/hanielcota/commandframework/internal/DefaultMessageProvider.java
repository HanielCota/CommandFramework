...
public final class DefaultMessageProvider implements MessageProvider {
    ...
    public String message(MessageKey key) {
        return switch (key) {
            ...
            case CONFIRM_PROMPT -> "<yellow>Click <click:run_command:'/{command}'>"
                    + "<hover:show_text:'<gray>Run /{command}'><green>[Confirm]</green></hover></click>"
                    + " within <white>{seconds}s</white>.";
            ...
            case UNKNOWN_SUBCOMMAND -> "<red>Unknown subcommand <yellow>{typed}</yellow>. Did you mean "
                    + "<click:suggest_command:'/{command} {suggestion}'><green>{suggestion}</green></click>?";
        };
    }
}
